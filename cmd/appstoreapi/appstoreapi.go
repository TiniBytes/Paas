...
import (
	...
	"net/http"
	"strconv"
	"tini-paas/api/appstoreapi/handler"
	"tini-paas/api/appstoreapi/proto/appstoreApi"
	microAppStoreService "tini-paas/internal/appstore/proto/appstore"
...
func main() {
	...
	// 2、添加链路追踪
	tracer, closer, err := common.NewTracer("go.micro.api.appstoreApi", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		common.Error(err)
	}
	defer closer.Close()
	...
	storeService := microAppStoreService.NewAppStoreService("go.micro.service.appStore", service.Client())
	err = appstoreApi.RegisterAppStoreApiHandler(service.Server(), &handler.AppStoreApi{
