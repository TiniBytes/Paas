...
import (
	...
	"strconv"
	"tini-paas/config"
	"tini-paas/internal/user/handler"
	...
	service2 "tini-paas/internal/user/service"
...
func main() {
	...
	// 4、添加链路追踪
	tracer, closer, err := common.NewTracer("go.micro.service.user", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		common.Error(err)
	}
	defer closer.Close()
	...
	// 注册句柄
	userService := service2.NewUserService(repository.NewUserRepository(db), clientSet)
	roleService := service2.NewRoleService(repository.NewRoleRepository(db), clientSet)
	permissionService := service2.NewPermissionService(repository.NewPermissionRepository(db), clientSet)
	err = user.RegisterUserHandler(service.Server(), &handler.UserHandler{
	...
	permission.RegisterPermissionHandler(service.Server(), &handler.PermissionHandler{
		PermissionDataService: permissionService,
	})
