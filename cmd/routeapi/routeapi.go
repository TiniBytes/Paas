...
import (
	...
	"net/http"
	"strconv"
	"tini-paas/api/routeapi/handler"
	"tini-paas/api/routeapi/proto/routeApi"
	microRouteService "tini-paas/internal/route/proto/route"
...
func main() {
	...
	// 2、添加链路追踪
	tracer, closer, err := common.NewTracer("go.micro.api.avcAPi", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		common.Error(err)
	}
	defer closer.Close()
	...
	routeService := microRouteService.NewRouteService("go.micro.service.route", service.Client())
	err = routeApi.RegisterRouteApiHandler(service.Server(), &handler.RouteApi{
