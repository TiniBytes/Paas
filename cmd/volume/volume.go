...
import (
	...
	"strconv"
	"tini-paas/config"
	"tini-paas/internal/volume/handler"
	"tini-paas/internal/volume/proto/volume"
	"tini-paas/internal/volume/repository"
	service2 "tini-paas/internal/volume/service"
...
func main() {
	...
	// 4、添加链路追踪
	tracer, closer, err := common.NewTracer("go.micro.service.svcapi", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		common.Error(err)
	}
	defer closer.Close()
	...
	// 注册句柄
	volumeService := service2.NewVolumeService(repository.NewVolumeRepository(db), clientSet)
	err = volume.RegisterVolumeHandler(service.Server(), &handler.VolumeHandler{
