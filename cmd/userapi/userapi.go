...
import (
	...
	"net/http"
	"strconv"
	"tini-paas/api/userapi/handler"
	"tini-paas/api/userapi/proto/userApi"
	microRoleService "tini-paas/internal/user/proto/role"
	microUserService "tini-paas/internal/user/proto/user"
...
func main() {
	...
	// 2、添加链路追踪
	tracer, closer, err := common.NewTracer("go.micro.api.userApi", tracerHost+":"+strconv.Itoa(tracerPort))
	if err != nil {
		common.Error(err)
	}
	defer closer.Close()
	...
	userService := microUserService.NewUserService("go.micro.service.user", service.Client())
	roleService := microRoleService.NewRoleService("go.micro.service.user", service.Client())
	err = userApi.RegisterUserApiHandler(service.Server(), &handler.UserApi{
